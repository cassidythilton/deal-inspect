...
-- Table 12: ACCOUNT_INTEL_SUMBLE_ORG (Sprint 6.5 — Tier 2: Organization firmographics)
...
-- Table 13: ACCOUNT_INTEL_SUMBLE_JOBS (Sprint 6.5 — Tier 3: Job posting intelligence)
...
-- Table 14: ACCOUNT_INTEL_SUMBLE_PEOPLE (Sprint 6.5 — Tier 4: Key people data)
...
-- Table 15: TDR_STRUCTURED_EXTRACTS — Cortex-extracted analytical fields (Sprint 17.5)
CREATE TABLE IF NOT EXISTS TDR_STRUCTURED_EXTRACTS (
  EXTRACT_ID           VARCHAR PRIMARY KEY,
  SESSION_ID           VARCHAR NOT NULL UNIQUE,    -- FK → TDR_SESSIONS (one extract per session)
  OPPORTUNITY_ID       VARCHAR NOT NULL,

  -- Tier 1: Direct from select/text fields
  THESIS               VARCHAR,
  STRATEGIC_VALUE      VARCHAR,
  CLOUD_PLATFORM       VARCHAR,
  ENTRY_LAYER          VARCHAR,
  DECISION_TIMELINE    VARCHAR,
  PARTNER_NAME         VARCHAR,
  PARTNER_POSTURE      VARCHAR,
  AI_MATURITY          VARCHAR,
  VERDICT              VARCHAR,

  -- Tier 2: Cortex AI extracted from free-text
  NAMED_COMPETITORS    VARIANT,
  NAMED_TECHNOLOGIES   VARIANT,
  NAMED_STAKEHOLDERS   VARIANT,
  RISK_CATEGORIES      VARIANT,
  DOMO_USE_CASES       VARIANT,
  ARCHITECTURAL_PATTERN VARCHAR,
  DEAL_COMPLEXITY      VARCHAR,
  KEY_DIFFERENTIATORS  VARIANT,
  CUSTOMER_DECISION_TYPE VARCHAR,
  URGENCY_DRIVERS      VARIANT,
...
-- View: V_TDR_ANALYTICS — single analytical surface joining sessions + extracts + enrichment
