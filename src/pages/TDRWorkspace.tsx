...
import { TDRInputs, isStepAutoComplete } from '@/components/TDRInputs';
...
export default function TDRWorkspace() {
  ...
  const [steps, setSteps] = useState<TDRStep[]>(
    tdrSteps.map((s, i) => ({ ...s, isComplete: false, isActive: i === 0 }))
  );
  ...
  // ── Semi-automated step completion ──
  useEffect(() => {
    if (!inputValues || !session) return;
    for (const step of stepsWithCompletion) {
      const getValue = (fieldId: string) => inputValues.get(`${step.id}::${fieldId}`) || '';
      const shouldComplete = isStepAutoComplete(step.id, getValue);
      if (shouldComplete && !completedSteps.has(step.id)) {
        markStepComplete(step.id);
      }
    }
  }, [inputValues, stepsWithCompletion, completedSteps, markStepComplete, session]);
