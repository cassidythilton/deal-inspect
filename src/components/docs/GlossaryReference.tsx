  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 gap-px rounded-lg border border-white/[0.08] overflow-hidden">
          {GLOSSARY.map((item, i) => (
            <div
              key={i}
              className="flex gap-4 px-4 py-2.5 bg-white/[0.02] hover:bg-white/[0.04] transition-colors"
            >
      ...
      <div className="space-y-3">
        ...
        <Accordion type="multiple" className="space-y-2">
          {FAQ.map((item, i) => (
            <AccordionItem key={i} value={`faq-${i}`} className="border border-white/[0.08] rounded-lg overflow-hidden">
              <AccordionTrigger className="px-4 py-3 text-sm font-medium text-white hover:bg-white/[0.03] [&[data-state=open]]:bg-white/[0.04] text-left">
                {item.question}
              </AccordionTrigger>
              <AccordionContent className="px-4 pb-3 text-sm text-slate-300 leading-relaxed">
                {item.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
